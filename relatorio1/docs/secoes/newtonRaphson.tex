...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Gráfico representativo do amortecimento logarítmico para $f_1(T)$. Nota-se que, distintamente dos métodos lineares (cujas retas de erro possuem declividade constante), a curva aqui sofre uma inflexão convexa para baixo, revelando a aceleração de segunda ordem onde os algarismos significativos multiplicam-se a cada passo.}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Gráfico de convergência atestando a extrema eficiência iterativa de Newton-Raphson na avaliação trigonométrica. O alcance do patamar de $10^{-6}$ em apenas 3 passos reflete a assertividade da aproximação linear na vizinhança correta da raiz.}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Gráfico comparativo da convergência para ambos os planos de financiamento. A trajetória essencialmente vertical em escala semi-logarítmica, extinguindo o erro em 3 iterações, destoa substancialmente do arrasto sublinear outrora documentado pelo Método da Posição Falsa, sagrando Newton-Raphson como a ferramenta primária para este perfil topológico.}
