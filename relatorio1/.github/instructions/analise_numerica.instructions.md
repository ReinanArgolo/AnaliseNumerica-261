...
- **Pipeline de Dados (I/O)**: Parametrizações locais (tolerância, iterações máximas) e arrays devem ser mantidos como arquivos estáticos (ASCII/CSV) em `dados/entradas/`. O histórico em iterações de cada teste/método deve sempre ser exportado num formato colunar como `.csv` na pasta `dados/saidas/`.
- **Integração Python $\rightarrow$ LaTeX**: Abandone transcrições manuais. O documento `.tex` em `docs/` deve consumir dinamicamente as tabelas em `dados/saidas/`.
